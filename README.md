...
This tool removes files in an upperdir from an Overlay Filesystem while keeping some files. The files to keep can be specified by multiple config files. It is intended for a cleanup whenever one or more lowerdirs have been updated to keep a minimal set of files. The overlayfs does not have to be mounted, only the lowerdir stack and the upperdir must be available.
...
* The contents of the lowerdir stack and upperdir directory are not changed by any other process while this tool is running.
...
```
overlayfs-purge -f [<keep-file> <keep-dir> <lower-dir>[:<lower-dir>...] <upper-dir>]
```
...
When positional arguments are supplied, the `lower-dir` argument may be a colon-separated list of lower directories. The directories are searched from left to right, and the first directory containing a given path is treated as the effective lowerdir entry for that path.
...
* Every other regular file, special file or directory in the upperdir matching a glob pattern is kept and its attributes and extended attributes are unchanged (see exception below).
* For every non-matching directory which cannot be purged because it contains matching components, if any lowerdir contains a directory at the same path, all attributes (user, group and access permissions, but not times) and extended attributes from the first such lowerdir in the configured lowerdir list are copied to the directory in the upperdir.
